            ...
            <div className='card' key={prod._id}>
              {isAuthenticated ? (
                wishlist.some((item) => item._id === prod._id) ? (
                  ...
                  <i
                    className='bi bi-heart wishlist'
                    onClick={() => {
                      addWishlist(prod);
                    }}
                  ></i>
              ...
              <div className='card-body'>
                ...
                {prod.discount ? (
                  <div className='price'>
                    Rs {prod.price * (1 - prod.discount / 100)}
                ...
                {isAuthenticated ? (
                  <Link to='/product'>
                    {cart.item.some((item) => item._id === prod._id) ? (
                      <>
                        <Link to='/cart'>
                          <button className='btn btn-secondary width-100 mt-1'>
                            Go to Cart
                          </button>
                        </Link>
                      </>
                      ...
                      <>
                        <button
                          className='btn btn-primary width-100 mt-1'
                          onClick={() => {
                            addCartItem(prod);
                          }}
                        >
